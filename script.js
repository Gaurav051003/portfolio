...
document.getElementById("changeColorBtn").addEventListener("click", () => {
  ...
  // Random fluorescent color select
  const keys = Object.keys(fluorescentColors);
  const randomKey = keys[Math.floor(Math.random() * keys.length)];
  const newColor = fluorescentColors[randomKey];

  // Ghost body emissive color update
  ghostMaterial.emissive = new THREE.Color(newColor);

  console.log("Ghost color changed to:", randomKey);
});
